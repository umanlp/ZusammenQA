...
target_langs =  ['ar', 'bn', 'fi', 'ja', 'ko', 'ru', 'te', 'en', 'es', 'km', 'ms', 'ru', 'sv', 'tr', 'zh_cn', 'ta', 'tl']
...
def main():
    ...
    parser.add_argument("--aug_fp", default=None, type=str)
    ...
    if args.aug_fp is not None:
        aug_data = json.load(open(args.aug_fp))
    ...
    if args.train_fp is not None:
        ...
        source_f_train = open(os.path.join(
            args.output_dir, "train_new.source"), "w")
        target_f_train = open(os.path.join(
            args.output_dir, "train_new.target"), "w")
        
        if args.aug_fp is not None:
            s2s_aug = load_dpr_results(aug_data, top_n=args.top_n)
            end_data = s2s_train+s2s_aug
            random.shuffle(end_data)
            print(len(end_data))
        else:
            end_data = s2s_train
            print(len(end_data))
